  ...
  .then(data => {
    ...
    fetch('https://api.openweathermap.org/data/2.5/onecall?lat=' + data.coord.lat + '&lon=' + data.coord.lon + '&exclude=alerts&units=imperial&appid=ea7c6aed441b6888e5a12f5ec4d6ed26')
      ...
      .then(data => {
        ...
        tempnow.textContent = "Temperatura: " + data.current.temp + ' Farenheit'
        windnow.textContent = "Velocidad del viento: " + data.current.wind_speed + 'mph'
        humidnow.textContent = "Humedad: " + data.current.humidity + "%"
        UVNow.textContent = "Índice UV : " + data.current.uvi
        ...
        for (i = 1; i < 6; i++) {
          ...
          document.querySelector('#humid' + i).textContent = 'Humedad: ' + data.daily[i].humidity + '%'
          document.querySelector('#wind' + i).textContent = 'Velocidad del viento: ' + data.daily[i].wind_speed + "MPH"
      ...
      .then(data => {
        ...
        fetch('https://api.openweathermap.org/data/2.5/onecall?lat=' + data.coord.lat + '&lon=' + data.coord.lon + '&exclude=alerts&units=imperial&appid=ea7c6aed441b6888e5a12f5ec4d6ed26')
          ...
          .then(data => {
            ...
            windnow.textContent = "Velocidad del viento: " + data.current.wind_speed + 'mph'
            humidnow.textContent = "Humedad: " + data.current.humidity + "%"
            UVNow.textContent = "Indice UV : " + data.current.uvi
